...
## Lifecycle, durability & failure modes

`open` takes an opts map (both keys optional):

```clojure
(r/open "state.edn")                                  ; baseline {}, torn tail surfaced
(r/open "state.edn" {:baseline {:status :init}})      ; fold the log onto this instead of {}
(r/open "state.edn" {:on-torn-tail :discard})         ; self-heal a crash-torn tail
```

- **`:baseline`** (default `{}`) — the value the log is replayed onto. Root state
  is arbitrary EDN, so seed `[]`, a record, or any value; open replays with the
  *same* `apply-patch` fold the drainer uses (one code path, memory and disk). An
  explicit `nil` is indistinguishable from absent and gets the default. A missing
  file (and any missing parent dirs) opens a fresh db at `baseline`.
- **`:on-torn-tail`** — a crash mid-append can leave a malformed trailing record.
  `:surface` (default) throws on open with the byte offset and raw bytes/text of
  the lost in-flight tx in the ex-data's `:dj.recorder/torn-tail` (inspect
  `:text` for the fragment, `:bytes` for raw), leaving the file intact so you can
  decide. `:discard` truncates back to the last good record and proceeds. The
  good prefix is always replayed either way — only the partial tail is at stake.

**Single-writer lock.** `open` acquires an exclusive file lock and holds it for
the db's life; a second `open` on the same path (another process, or an unclosed
handle in this one) **fails fast** — it throws immediately rather than blocking.

**Halt-on-I/O-failure (§5).** If a durable append fails (disk full, I/O error),
the db *halts*: it stays `deref`-able (`@db` still returns the last good state)
but every further write throws. The failing tx's promise throws the underlying
error on deref; `(r/error db)` returns the halting `Throwable` (nil while live),
future-style, without you having to unwrap a promise. Recover by `close!` +
re-`open`.

**`await` vs. deref.** `@(r/patch! …)` is the per-tx barrier (blocks until *that*
write is durable). `(r/await db)` is the queue barrier — it returns only once
every write enqueued before the call is durable (agent-style), and throws if the
db is halted or closed.

**`close!` (§6).** Drains in-flight work, closes the writer, releases the lock.
**Idempotent** — first call wins, later calls no-op. Halt-safe (a halted db still
closes cleanly; unlike `await`, close never throws on a halt). After close,
`patch!`/`tx!`/`await` throw, but `@db` still works — the handle becomes an
immutable view of its last durable state. A db prints as
`#dj.recorder/db {:path … :halted? … :closed? …}` for quick REPL inspection.

## Using dj.recorder as a durable backend (adapters)

An adapter lets *another* library use an open db as its durable store by
implementing that library's protocol (cf. `ring.adapter.*`; see Layout). The
shipped example, `dj.recorder.adapter.concurrency`, exposes a db as a
dj.concurrency `ResultStore` so an LLM/pipeline supervisor resumes from the log
after a crash:
...
(def db    (r/open "runs.edn"))
(def store (adapter/recorder-store db [:results "run-42"]))  ; namespace entries under a path-prefix
```

`recorder-store` reads are instant in-memory `get-in`s; `record!`/`evict!` deref
the per-tx promise, so they return only once durable (persist-then-publish
upstream). The whole adapter is ~20 lines over the public API — a template for
adapting any store-shaped protocol.
